...
//tv
export async function checkBookMark_TV(id) {
  const id_user_collection = await ConsultaID();
  let result = undefined;
  const docRef = db.collection("bookmark_TV").doc(id_user_collection);
...
export async function deleteBookMark_TV(id) {
  const id_user_collection = await ConsultaID();
  return db
    .collection("bookmark_TV")
    .doc(id_user_collection)
    .update({
      id_movie: a.firestore.FieldValue.arrayRemove({ id }),
    });
...
export async function saveBookMark_TV(id) {
  const id_user_collection = await ConsultaID();
  return db
    .collection("bookmark_TV")
    .doc(id_user_collection)
