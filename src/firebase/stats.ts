...
//tv
export async function getGenreStadsTv() {
  let result = undefined;
  const id_user_collection = await ConsultaID();

  const docRef = db.collection("eye_TV").doc(id_user_collection);
  await docRef
    .get()
    .then((doc) => {
      if (doc.exists) {
...
export async function getDateReleaseTv() {
  let result = undefined;
  const id_user_collection = await ConsultaID();
...
export async function getTimeStatsYearTv() {
  let result = undefined;
  const id_user_collection = await ConsultaID();
...
export async function getTimeStatsWeekTv() {
  let result = undefined;
  const id_user_collection = await ConsultaID();
